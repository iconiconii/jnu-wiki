...
export async function POST(request: NextRequest) {
  try {
    ...
    // 长度验证
    if (title.length > 100) {
      return NextResponse.json({ error: '标题长度不能超过100字符' }, { status: 400 })
    }

    if (content.length > 1000) {
      return NextResponse.json({ error: '反馈内容长度不能超过1000字符' }, { status: 400 })
    }
    ...
    if (containsSensitiveWords(cleanTitle) || containsSensitiveWords(cleanContent)) {
      return NextResponse.json({ error: '反馈内容包含不当信息，请修改后重试' }, { status: 400 })
    }

    // 最小长度验证
    if (cleanTitle.length < 5 || cleanContent.length < 10) {
      return NextResponse.json({ error: '标题至少5个字符，内容至少10个字符' }, { status: 400 })
    }
...
export async function GET(request: NextRequest) {
  try {
    ...
    // 获取统计数据
    const { data: statsData } = await supabaseAdmin.rpc('get_feedback_stats')
